    ...
    // 🔥 Step 1: Send selected answers to Flask for prediction
    try {
        ...
        const predictedMood = predictionData.predicted_mood;
        moodResult.innerText = `Your mood: ${predictedMood}`; // show mood

        // 🔥 Step 2: Send mood to get song recommendations
        const songsResponse = await fetch('/get_songs', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ mood: predictedMood })
        });

        const songsData = await songsResponse.json();
        const songs = songsData.songs;

        // 🔥 Step 3: Display the songs
        const songResults = document.getElementById('songResults');
        songResults.innerHTML = ''; // Clear any previous data
        ...
        songs.forEach(song => {
            ...
            songCard.innerHTML = `
                <img src="${song.image}" alt="cover" class="song-image">
                <div class="song-info">
        ...
        console.error("Error fetching mood or songs:", error);
        moodResult.innerText = "Oops! Something went wrong.";
        moodResult.style.display = "block";
    }
}
...
async function analyzeMood() {
    ...
    try {
        ...
        // Hide quiz result page, show mood result
        quizResultPage.style.display = "none";
        ...
        moodAnalysisText.innerHTML = `
            <div class="selected-option">
                <h3>${mood}</h3>
                <p>Based on your vibe, this is what your soul sounds like today 🎵</p>
            </div>
        `;
        ...
        // Get both songs
        const sitSong = songs.sit;
        const upliftSong = songs.uplift;

        [sitSong, upliftSong].forEach(song => {
            if (!song || !song.title) return; // Skip if empty
            const songCard = document.createElement("div");
            songCard.classList.add("song-card");
            songCard.innerHTML = `
        ...
        moodAnalysisText.innerHTML = `
            <p>Oops! Something went wrong while analyzing your mood.</p>
        `;
    }
}
